...
void SpamCard::Update()
{
	...
	if (curPlayer != GET_SINGLE(BoardManager)->GetCurrentPlayer()) //턴 바뀜
	{
		...
		for (int i = 0; i < 7; ++i)
		{
			...
			// 안전한 윈도우 키 생성: L"Spam" + i 형태의 포인터 산술 제거
			std::wstring spamKey = std::wstring(L"Spam") + std::to_wstring(i);
			...
			SpamCloseButton* closeButton = new SpamCloseButton(spamKey, { textureSize.x - 30.f, 0.f }, { 30.f,30.f });
